...
from typing import Any, Dict, List, Optional, Protocol, Text, Tuple, Union
...
class Frame:
    ...
    data: Union[
        np.ndarray,
        bytes]  # Raw data - either numpy array for images or bytes for pickled data
...
class StreamConfig:
    ...
    encoding: (
        str  # container encoding. rawvideo | libaom-av1 | ffv1 | libx264 | libx265
    )
    codec_options: Optional[Dict[str, Any]] = None
    pixel_format: Optional[str] = None
    width: Optional[int] = None
    height: Optional[int] = None
    internal_codec: Optional[str] = (
        None  # Internal codec implementation. pickle_raw | pyarrow_batch
    )
