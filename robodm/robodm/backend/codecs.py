        ...
        try:
            ...
            packet = CodecPacket(
                ...
                metadata={
                    ...
                    "data_dtype": (str(getattr(data, "dtype", None))
                                   if hasattr(data, "dtype") else None),
                },
    ...
    def encode(self, data: Any, timestamp: int, **kwargs) -> List[CodecPacket]:
        ...
        try:
            ...
            if isinstance(data, np.ndarray):
                ...
                data_info = {"type": "pickle", "data": pickle.dumps(data)}
    ...
    def _flush_batch(self) -> List[CodecPacket]:
        ...
        try:
            ...
            table = pa.table({
                ...
                "data_type":
                [entry["data_info"]["type"] for entry in self.current_batch],
                "data_shape": [
                    entry["data_info"].get("shape")
                    for entry in self.current_batch
                ],
                "data_dtype": [
                    entry["data_info"].get("dtype")
                    for entry in self.current_batch
                ],
                "data_bytes":
                [entry["data_info"]["data"] for entry in self.current_batch],
            })
...
class PyAVVideoCodec(VideoCodec):
            ...
            for pkt in self._stream.encode(frame):
                codec_packet = CodecPacket(
                    ...
                    metadata={
                        ...
                        "dts": pkt.dts,
                        "codec": self.codec_name,
                        "is_keyframe": bool(getattr(pkt, "is_keyframe",
                                                    False)),
                    },
    ...
    def decode(self, packet: CodecPacket) -> Any:
        ...
        raise NotImplementedError(
            "Video decoding is handled by container backend")
...
def register_codec(name: str, codec_class: type):
    ...
    if not issubclass(codec_class, DataCodec):
        raise TypeError(
            f"Codec class must inherit from DataCodec, got {codec_class}")
...
def get_codec(codec_name: str, **kwargs) -> DataCodec:
    ...
    if cache_key not in _codec_instances:
        if codec_name not in _codec_factories:
            raise ValueError(
                f"Unknown codec: {codec_name}. Available: {list(_codec_factories.keys())}"
            )
